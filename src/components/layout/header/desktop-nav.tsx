...
export default function DesktopNav() {
  ...
  const activeSectionId = useScrollSpy(sectionIds, 100); // 100px offset for header
    ...
    <nav className="hidden lg:flex lg:items-center bg-[#F9FAFB] dark:bg-white/3 rounded-full p-1 max-h-fit">
      {navItems.map((item) => {
        if (item.type === 'link') {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                'text-gray-500 dark:text-gray-400 text-sm px-4 py-1.5 rounded-full hover:text-primary-500 font-medium transition-all duration-300',
                {
        ...
        if (item.type === 'dropdown') {
          const dropdownItem = item; // Type narrowing
            ...
            <div key={dropdownItem.label} className="relative">
              <button
                ...
                className={cn(
                  ...
                  {
                    'bg-white dark:bg-white/5 font-medium text-gray-800 dark:text-white/90 shadow-xs':
                      dropdownItem.items.some(({ href }: { href: string }) => pathname?.includes(href)),
                  }
                )}
              >
              ...
              {isDropdownActive && (
                ...
                >
                  <div className="space-y-1">
                    {dropdownItem.items.map((subItem: { label: string; href: string; icon?: React.ReactNode }) => (
                      <Link
                        key={subItem.href}
                        href={subItem.href}
